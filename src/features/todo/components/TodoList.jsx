  ...
  const handleStatus = (todo) => {
    ...
    dispatch(updateTodoAsync(updatedTodo))
      .then(() => {
        ...
        toast.success("👍todo's status changed!");
      })
      .catch((error) => {
        console.log(error);
      });
      ...
      <div className="grid grid-cols-1 gap-x-6 gap-y-6 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 xl:gap-x-8">
        {currentTodos?.map((todo) => (
          <div key={todo.id} className="group relative z-10 shadow-lg rounded-lg bg-gray-200 group-hover:opacity-75">
            <div className="w-full h-40 p-2 md:p-4 overflow-hidden flex flex-col justify-between">
              <div className=" flex flex-col-reverse  sm:flex-row justify-end sm:justify-between gap-2">
                ...
                <span className={`${chooseStyle(todo.completed)} h-6 w-full flex justify-center items-center text-center sm:w-auto py-1 sm:px-6 rounded-full text-xs font-semibold`}>{todo.completed ? "Done" : "Active"}</span>
              </div>

              <div className="w-full flex flex-row justify-end items-center">
                <div className="w-1/3 flex flex-row justify-end items-center gap-2">
                  {todoId === todo.id && editMode ? (
                    <button onClick={() => handleSave(todo)} className=" flex h-6 w-6 flex-shrink-0 items-center justify-center rounded-full bg-yellow-100 sm:mx-0 " title="update">
